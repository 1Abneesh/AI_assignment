...
int Tilediffernce(vector<vector<int>> matrix, vector<vector<int>> goal_matrix){
    int count = 0;
...
int manhattan(vector<vector<int>> matrix, vector<vector<int>> goal_matrix){
    int count = 0;
...
void display(vector<vector<int>> matrix, vector<vector<int>> goal_matrix, int level, int type){
    ...
    if (type == 1){
        h = manhattan(matrix, goal_matrix);
    }
    else{
        h = Tilediffernce(matrix, goal_matrix);
    }
    cout<<"\nCost : "<<h<<"\n";
    cout<<"Heuristic value : "<<h+level<<"\n";
    cout<<"level : "<<level<<"\n";
}
...
vector<vector<int>> transform_matrix(vector<vector<int>> matrix, vector<vector<int>> goal_matrix, set<vector<vector<int>>> &matrix_set, int level, int type){
    ...
    for (int i = 0; i <matrix_collection.size(); i++){
        ...
        if (pos == matrix_set.end()){
            ...
            if (type == 1){
                temp = manhattan(matrix_collection[i], goal_matrix);
            }
            else{
                temp = Tilediffernce(matrix_collection[i], goal_matrix);
            }
...
int main(){
    ...
    set<vector<vector<int>>> matrix_set_1;
    matrix_set.insert(matrix);
    matrix_set_1 = matrix_set;
